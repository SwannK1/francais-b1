...
export default function GuidedSessionCard({
  ...
  locked?: boolean;
  /** Mise en avant en point d'entrée principal (`/parcours`) — même carte, présentation renforcée. */
  prominent?: boolean;
  ...
  return (
    <Card
      className={cn(
        prominent && "border-primary/30 bg-primary/[0.03] shadow-md sm:p-8"
      )}
    >
      ...
      <h3 className={cn("font-semibold text-foreground", prominent ? "text-xl sm:text-2xl" : "text-lg")}>
        {plan.moduleTitle}
      </h3>
      ...
      {!locked && typeof completionRate === "number" ? (
