import type { Exercise, Module, PublicModule } from "@/lib/pedagogy/types";
...
/**
 * Premier module (dans `modules`, déjà la vue publique reçue de l'appelant)
 * contenant un exercice de cette compétence — utilisé pour rendre une
 * compétence faible (`UserProgress.weakSkillIds`) actionnable dans la page
 * progression (un lien "s'entraîner", pas juste un constat). Prend
 * `PublicModule[]` : `PublicExercise.skillId` suffit à cette recherche,
 * jamais besoin du contenu protégé pour la mener.
 */
export function findModuleForSkill(modules: PublicModule[], skillId: string): PublicModule | undefined {
