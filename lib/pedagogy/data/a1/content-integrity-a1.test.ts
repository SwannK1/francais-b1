 ...
 *
 * La vérification "audioSrc existe sur disque" (alignée sur
 * `content-integrity.test.ts` § Pipeline audio humain) a été ajoutée par le
 * chantier `audio-humanisation` : le chantier `a1-audio` qui en différait
 * volontairement la vérification a depuis livré ses 65 fichiers, fermant le
 * trou de couverture documenté ici auparavant (une régression silencieuse —
 * fichier renommé/supprimé sans toucher ce test — serait désormais détectée).
 */
...
describe("A1 — Non-régression du mismatch la-sante (docs/audio/humanisation-a1-a2-b1.md §6.6)", () => {
  it("l'exercice m18-f1 (module La santé) porte réellement sur la douleur et le conseil médical, pas seulement une prise de rendez-vous", () => {
    const mod = MODULES_A1.find((m) => m.slug === "la-sante");
    expect(mod).toBeDefined();
    let exercise: Exercise | undefined;
    for (const l of mod!.lessons) {
      for (const a of l.activities) {
        const found = a.exercises.find((ex) => ex.id === "m18-f1");
    ...
    // Le vocabulaire réellement enseigné par le module (voir `vocabulary`
    // de "La santé" dans quotidien-et-loisirs.ts : avoir mal à, un
    // médicament) doit apparaître dans le transcript — pas seulement un
    // thème médical adjacent (prise de rendez-vous).
    const transcript = exercise!.transcript ?? "";
    expect(transcript).toMatch(/mal à/i);
    expect(transcript).toMatch(/médicament/i);
  });
});
