...
-- Statut premium (chantier commerce) : écrites uniquement par le webhook
-- Stripe (voir app/api/webhooks/stripe/route.ts), jamais par le client.
-- `ADD COLUMN IF NOT EXISTS` plutôt qu'un ajout dans le CREATE TABLE
-- ci-dessus : sur une base déjà provisionnée avant cet ajout, rejouer ce
-- fichier doit rester idempotent et ne pas échouer sur le CREATE TABLE.
